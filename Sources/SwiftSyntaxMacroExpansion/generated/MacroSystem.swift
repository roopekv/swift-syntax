...
private func expandAccessorMacroWithoutExistingAccessors(
  ...
  let indentedSource = " " + expanded.indented(by: attachedTo.indentationOfFirstLine).drop(while: {
    $0.isWhitespace
  })
  return "\(raw: indentedSource)"
}
...
private func expandBodyMacro(
  ...
  let indentedSource =
    leadingWhitespace + expanded.indented(by: decl.indentationOfFirstLine).drop(while: {
    $0.isWhitespace
  })
  return "\(raw: indentedSource)"
}
...
private class MacroApplication<Context: MacroExpansionContext>: SyntaxRewriter {
  ...
  override func visitAny(_ node: Syntax) -> Syntax? {
      ...
      !attributedNode.attributes.isEmpty
    {
      ...
      return AttributeRemover(removingWhere: {
        attributesToRemove.contains($0)
      }).rewrite(visitedNode)
    }
  ...
  override func visit(_ node: MemberBlockSyntax) -> MemberBlockSyntax {
    ...
    for var item in node.members {
      ...
      if let parentDeclGroup, let decl = item.decl.asProtocol(WithAttributesSyntax.self) {
        var attributes = expandAttributesFromMemberAttributeMacros(
          of: item.decl,
          parentDecl: parentDeclGroup
        )

        for i in 0 ..< attributes.count {
          attributes[i] = visit(attributes[i])
        }
...
extension MacroApplication {
  ...
  private func expandFreestandingMacro<ExpandedMacroType: SyntaxProtocol>(
    ...
    do {
      guard !expandingFreestandingMacros.contains(where: { $0 == macro
      }) else {
...
extension SyntaxProtocol {
  ...
  public func expand<Context: MacroExpansionContext>(
    ...
    let applier = AsyncMacroApplication(
      macroSystem: system,
      contextGenerator: contextGenerator,
      indentationWidth: indentationWidth
    )

    return await applier.rewrite(self)
  }
}
...
private func expandFreestandingMemberDeclList(
  ...
  in context: some MacroExpansionContext,
  indentationWidth: Trivia
) async throws -> MemberBlockItemListSyntax? {
  guard
    let expanded = try await expandFreestandingMacro(
      definition: definition,
      macroRole: inferFreestandingMacroRole(definition: definition),
      node: node.detach(in: context, foldingWith: .standardOperators),
      in: context,
...
private func expandFreestandingExpr(
  ...
  guard
    let expanded = await expandFreestandingMacro(
      definition: definition,
      macroRole: .expression,
      node: node.detach(in: context, foldingWith: .standardOperators),
      in: context,
...
private func expandMemberMacro(
  ...
  guard
    let expanded = await expandAttachedMacro(
      definition: definition,
      macroRole: .member,
      attributeNode: attributeNode.detach(in: context, foldingWith: .standardOperators),
      declarationNode: attachedTo.detach(in: context),
...
private func expandMemberAttributeMacro(
  ...
  in context: some MacroExpansionContext,
  indentationWidth: Trivia
) async throws -> AttributeListSyntax? {
  guard
    let expanded = await expandAttachedMacro(
      definition: definition,
      macroRole: .memberAttribute,
...
private func expandAccessorMacroWithoutExistingAccessors(
  ...
  in context: some MacroExpansionContext,
  indentationWidth: Trivia
) async throws -> AccessorBlockSyntax? {
  guard
    let expanded = await expandAttachedMacro(
      definition: definition,
      macroRole: .accessor,
...
private func expandAccessorMacroWithExistingAccessors(
  ...
  in context: some MacroExpansionContext,
  indentationWidth: Trivia
) async throws -> AccessorDeclListSyntax? {
  guard
    let expanded = await expandAttachedMacro(
      definition: definition,
      macroRole: .accessor,
...
// MARK: - AsyncMacroApplication

/// Syntax rewriter that evaluates any macros encountered along the way.
private class AsyncMacroApplication<Context: MacroExpansionContext>: AsyncSyntaxRewriter {
  let macroSystem: MacroSystem
  ...
  override func visitAny(_ node: Syntax) async -> Syntax? {
    guard !skipVisitAnyHandling.contains(node) else {
      return nil
    }
    ...
    // position are handled by 'visit(_:CodeBlockItemListSyntax)'.
    // Only expression expansions inside other syntax nodes is handled here.
    switch await expandExpr(node: node) {
    case .success(let expansion):
      return await expansion.withExpandedNode { expandedNode in
        Syntax(await visit(expandedNode))
      }
    case .failure:
      return Syntax(node)
    case .notAMacro:
      ...
      !attributedNode.attributes.isEmpty
    {
      ...
      if let nodeWithBody = node.asProtocol(WithOptionalCodeBlockSyntax.self),
        let declNodeWithBody = nodeWithBody as? any DeclSyntaxProtocol & WithOptionalCodeBlockSyntax
      {
        declSyntax = await DeclSyntax(visitBodyAndPreambleMacros(declNodeWithBody))
      }

      // Visit the node, disabling the `visitAny` handling.
      skipVisitAnyHandling.insert(Syntax(declSyntax))
      let visitedNode = await self.visit(declSyntax)
      skipVisitAnyHandling.remove(Syntax(declSyntax))
  ...
  func visitBodyAndPreambleMacros<Node: DeclSyntaxProtocol & WithOptionalCodeBlockSyntax>(
    ...
    // Expand preamble macros into a set of code items.
    let preamble = await expandMacros(
      attachedTo: DeclSyntax(node),
      ofType: PreambleMacro.Type.self
    ) { attributeNode, definition, _ in
      await expandPreambleMacro(
        definition: definition,
        attributeNode: attributeNode,
        attachedTo: node,
        in: contextGenerator(Syntax(node)),
    ...
    // Expand body macro.
    let expandedBodies = await expandMacros(
      attachedTo: DeclSyntax(node),
      ofType: BodyMacro.Type.self
    ) { attributeNode, definition, _ in
      await expandBodyMacro(
        definition: definition,
        attributeNode: attributeNode,
        attachedTo: node,
        in: contextGenerator(Syntax(node)),
  ...
  override func visit(_ node: CodeBlockItemListSyntax) async -> CodeBlockItemListSyntax {
    var newItems: [CodeBlockItemSyntax] = []
    func addResult(_ node: CodeBlockItemSyntax) async {
      // Expand freestanding macro.
      switch await expandCodeBlockItem(node: node) {
      case .success(let expansion):
        await expansion.withExpandedNode { expandedNode in
          for item in expandedNode {
      ...
      case .notAMacro:
        // Recurse on the child node
        newItems.append(await visit(node))
      }

      // Expand any peer macro on this item.
      if case .decl(let decl) = node.item {
        for peer in await expandCodeBlockPeers(of: decl) {
          await addResult(peer)
        }
        extensions += await expandExtensions(of: decl)
      }
    }
  ...
  override func visit(_ node: MemberBlockSyntax) async -> MemberBlockSyntax {
    let parentDeclGroup = node
      .parent?
      .as(DeclSyntax.self)
    var newItems: [MemberBlockItemSyntax] = []

    func addResult(_ node: MemberBlockItemSyntax) async {
      // Expand freestanding macro.
      switch await expandMemberDecl(node: node) {
      case .success(let expansion):
        await expansion.withExpandedNode { expandedNode in
          for item in expandedNode {
            await addResult(item)
      ...
      // Expand any peer macro on this member.
      for peer in await expandMemberDeclPeers(of: node.decl) {
        await addResult(peer)
      }
      extensions += await expandExtensions(of: node.decl)
    }
    ...
    for var item in node.members {
      ...
      // Note that MemberAttribute macros are _not_ applied to generated members
      if let parentDeclGroup, let decl = item.decl.asProtocol(WithAttributesSyntax.self) {
        var attributes = await expandAttributesFromMemberAttributeMacros(
          of: item.decl,
          parentDecl: parentDeclGroup
        )

        for i in 0 ..< attributes.count {
          attributes[i] = await visit(attributes[i])
        }
    ...
    // Expand any member macros of parent.
    if let parentDeclGroup {
      for member in await expandMembers(of: parentDeclGroup) {
        await addResult(member)
      }
    }
  ...
  override func visit(_ node: VariableDeclSyntax) async -> DeclSyntax {
    var node = await super.visit(node).cast(VariableDeclSyntax.self)
    ...
    var expansion = await expandAccessors(of: node, existingAccessors: binding.accessorBlock)
  ...
  override func visit(_ node: SubscriptDeclSyntax) async -> DeclSyntax {
    var node = await super.visit(node).cast(SubscriptDeclSyntax.self)
    node.accessorBlock = await expandAccessors(of: node, existingAccessors: node.accessorBlock).accessors
    return DeclSyntax(node)
  }
}
...
extension AsyncMacroApplication {
  /// Get pairs of a macro attribute and the macro specification attached to `decl`.
  ///
  /// The macros must be registered in `macroSystem`.
  private func macroAttributes(
  ...
  >(
    ...
    expandMacro:
      (
        ...
        _ definition: MacroType,
        _ conformanceList: InheritedTypeListSyntax
      ) async throws -> ExpandedNodeCollection?
  ) async -> [ExpandedNode] {
    var result: [ExpandedNode] = []

    for macroAttribute in macroAttributes(attachedTo: decl, ofType: ofType) {
      do {
        if let expanded = try await expandMacro(
          macroAttribute.attributeNode,
          macroAttribute.definition,
          macroAttribute.conformanceList
        ) {
  ...
  ///
  /// - Returns: The macro-synthesized peers
  private func expandMemberDeclPeers(of decl: DeclSyntax) async -> [MemberBlockItemSyntax] {
    return await expandMacros(attachedTo: decl, ofType: PeerMacro.Type.self) { attributeNode, definition, conformanceList in
      return try await expandPeerMacroMember(
        definition: definition,
        attributeNode: attributeNode,
        attachedTo: decl,
        in: contextGenerator(Syntax(decl)),
  ...
  ///
  /// - Returns: The macro-synthesized peers
  private func expandCodeBlockPeers(of decl: DeclSyntax) async -> [CodeBlockItemSyntax] {
    return await expandMacros(attachedTo: decl, ofType: PeerMacro.Type.self) { attributeNode, definition, conformanceList in
      return try await expandPeerMacroCodeItem(
        definition: definition,
        attributeNode: attributeNode,
        attachedTo: decl,
        in: contextGenerator(Syntax(decl)),
  ...
  ///
  /// - Returns: The macro-synthesized extensions
  private func expandExtensions(of decl: DeclSyntax) async -> [CodeBlockItemSyntax] {
    return await expandMacros(
      attachedTo: decl,
      ofType: ExtensionMacro.Type.self
    ) { attributeNode, definition, conformanceList in
      return try await expandExtensionMacro(
        definition: definition,
        attributeNode: attributeNode,
        attachedTo: decl,
        conformanceList: conformanceList,
  ...
  /// Expand all 'member' macros attached to `decl`.
  private func expandMembers(of decl: DeclSyntax) async -> [MemberBlockItemSyntax] {
    return await expandMacros(attachedTo: decl, ofType: MemberMacro.Type.self) { attributeNode, definition, conformanceList in
      return try await expandMemberMacro(
        definition: definition,
        attributeNode: attributeNode,
        attachedTo: decl,
        conformanceList: conformanceList,
  ...
  private func expandAttributesFromMemberAttributeMacros(
    of decl: DeclSyntax,
    parentDecl: DeclSyntax
  ) async -> [AttributeListSyntax.Element] {
    return await expandMacros(
      attachedTo: parentDecl,
      ofType: MemberAttributeMacro.Type.self
    ) { attributeNode, definition, conformanceList in
      return try await expandMemberAttributeMacro(
        definition: definition,
        attributeNode: attributeNode,
        attachedTo: parentDecl,
        providingAttributeFor: decl,
  ...
  private func expandAccessors(
    of storage: some DeclSyntaxProtocol,
    existingAccessors: AccessorBlockSyntax?
  ) async -> (accessors: AccessorBlockSyntax?, expandsGetSet: Bool) {
    let accessorMacros = macroAttributes(attachedTo: DeclSyntax(storage), ofType: AccessorMacro.Type.self)
    ...
    for macro in accessorMacros {
      do {
        ...
        // based on these cases.
        if existingAccessors != nil {
          if let newAccessors = try await expandAccessorMacroWithExistingAccessors(
            definition: macro.definition,
            attributeNode: macro.attributeNode,
            attachedTo: DeclSyntax(storage),
            in: contextGenerator(Syntax(storage)),
        ...
        } else if let newAccessors = try await expandAccessorMacroWithoutExistingAccessors(
          definition: macro.definition,
          attributeNode: macro.attributeNode,
          attachedTo: DeclSyntax(storage),
          in: contextGenerator(Syntax(storage)),
...
extension AsyncMacroApplication {
  ...
  struct MacroExpansion<ResultType> {
    ...
    private unowned let macroApplication: AsyncMacroApplication

    fileprivate init(expandedNode: ResultType, macro: any Macro.Type, macroApplication: AsyncMacroApplication) {
      self.expandedNode = expandedNode
      self.macro = macro
      self.macroApplication = macroApplication
    }
    ...
    /// an exact stack of expanding freestanding macros to detect recursive macro expansion. Callers should perform any
    /// further macro expansion on `expanded` only within the scope of `body`.
    func withExpandedNode<T>(_ body: (_ expandedNode: ResultType) async throws -> T) async rethrows -> T {
      macroApplication.expandingFreestandingMacros.append(macro)
      defer {
        macroApplication.expandingFreestandingMacros.removeLast()
      }
      return try await body(expandedNode)
    }
  }
  ...
  private func expandFreestandingMacro<ExpandedMacroType: SyntaxProtocol>(
    _ node: (any FreestandingMacroExpansionSyntax)?,
    expandMacro: (_ macro: any Macro.Type, _ node: any FreestandingMacroExpansionSyntax) async throws -> ExpandedMacroType?
  ) async -> MacroExpansionResult<ExpandedMacroType> {
    guard let node,
      let macro = macroSystem.lookup(node.macroName.text)?.type
    else {
      return .notAMacro
    ...
    do {
      ...
      if let expanded = try await expandMacro(macro, node) {
        return .success(expansion: MacroExpansion(expandedNode: expanded, macro: macro, macroApplication: self))
      } else {
        return .failure
      }
  ...
  /// }
  /// ```
  func expandCodeBlockItem(node: CodeBlockItemSyntax) async -> MacroExpansionResult<CodeBlockItemListSyntax> {
    return await expandFreestandingMacro(node.item.asProtocol(FreestandingMacroExpansionSyntax.self)) { macro, node in
      return try await expandFreestandingCodeItemList(
        definition: macro,
        node: node,
        in: contextGenerator(Syntax(node)),
        indentationWidth: indentationWidth
  ...
  /// }
  /// ```
  func expandMemberDecl(node: MemberBlockItemSyntax) async -> MacroExpansionResult<MemberBlockItemListSyntax> {
    return await expandFreestandingMacro(node.decl.as(MacroExpansionDeclSyntax.self)) { macro, node in
      return try await expandFreestandingMemberDeclList(
        definition: macro,
        node: node,
        in: contextGenerator(Syntax(node)),
        indentationWidth: indentationWidth
  ...
  /// let a = #foo
  /// ```
  func expandExpr(node: Syntax) async -> MacroExpansionResult<ExprSyntax> {
    return await expandFreestandingMacro(node.as(MacroExpansionExprSyntax.self)) { macro, node in
      return try await expandFreestandingExpr(
        definition: macro,
        node: node,
        in: contextGenerator(Syntax(node)),
        indentationWidth: indentationWidth
