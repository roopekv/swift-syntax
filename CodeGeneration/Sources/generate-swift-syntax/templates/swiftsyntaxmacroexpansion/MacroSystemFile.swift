...
private func makeMacroSystemFile() -> SourceFileSyntax {
  return SourceFileSyntax(leadingTrivia: copyrightHeader) {
        ...
        IfConfigClauseSyntax(
          ...
          elements: .statements(
            CodeBlockItemListSyntax {
              DeclSyntax("internal import SwiftDiagnostics")
              DeclSyntax("internal import SwiftOperators")
              DeclSyntax("@_spi(MacroExpansion) internal import SwiftParser")
              DeclSyntax("@_spi(MacroExpansion) public import SwiftSyntax")
              DeclSyntax("internal import SwiftSyntaxBuilder")
        ...
        IfConfigClauseSyntax(
          ...
          elements: .statements(
            CodeBlockItemListSyntax {
              ...
              DeclSyntax("import SwiftOperators")
              DeclSyntax("@_spi(MacroExpansion) import SwiftParser")
              DeclSyntax("@_spi(MacroExpansion) import SwiftSyntax")
              DeclSyntax("import SwiftSyntaxBuilder")
    ...
    makeMacroExpansionCodePart1(isAsync: false)
    ...
    makeMacroExpansionCodePart2(isAsync: false)
    ...
    try! StructDeclSyntax(
      ...
      DeclSyntax("var macros: [String: MacroSpec] = [:]")
    ...
    try! ClassDeclSyntax(
      ...
      DeclSyntax("let predicate: (AttributeSyntax) -> Bool")

      DeclSyntax("var triviaToAttachToNextToken: Trivia = Trivia()")
    ...
    try! ExtensionDeclSyntax("private extension Trivia") {
      DeclSyntax(
        """
        func trimmingPrefix(
    ...
    DeclSyntax(#"let diagnosticDomain: String = "SwiftSyntaxMacroExpansion""#)

    try! EnumDeclSyntax("private enum MacroApplicationError: DiagnosticMessage, Error") {
      DeclSyntax("case accessorMacroOnVariableWithMultipleBindings")
      DeclSyntax("case peerMacroOnVariableWithMultipleBindings")
      DeclSyntax("case malformedAccessor")
      ...
      DeclSyntax("var severity: DiagnosticSeverity { return .error }")
    ...
    makeMacroApplicationCode(isAsync: false)
    ...
    try! ExtensionDeclSyntax("private extension String") {
      DeclSyntax(
        """
    ...
    try! ExtensionDeclSyntax("private extension SyntaxProtocol") {
      DeclSyntax(
        """
    ...
    try! ExtensionDeclSyntax("private extension FreestandingMacroExpansionSyntax") {
      DeclSyntax(
        """
    ...
    try! ExtensionDeclSyntax("private extension AttributeSyntax") {
      DeclSyntax(
        """
        /// Same as `SyntaxProtocol.detach(in:foldingWith:)` but returns a node of type
        /// `Self` since we know that operator folding doesn't change the type of any
    ...
    try! ExtensionDeclSyntax("private extension AccessorDeclSyntax") {
      DeclSyntax(
        """
        var isGetOrSet: Bool {
    ...
    makeMacroExpansionCodePart1(isAsync: true)
    makeMacroExpansionCodePart2(isAsync: true)
    makeMacroApplicationCode(isAsync: true)
  }
}

private func makeMacroExpansionCodePart1(isAsync: Bool) -> CodeBlockItemListSyntax {
  let namePrefix = if isAsync { "Async" } else { String() }
  ...
  return CodeBlockItemListSyntax() {
    try! ExtensionDeclSyntax(
      ...
      DeclSyntax(
        ...
        )\(raw: asyncSpecifier) -> Syntax {
          return \(raw: awaitOperator)expand(
            macros: macros,
      ...
      DeclSyntax(
        ...
        public func expand<Context: MacroExpansionContext>(
          ...
          let applier = \(raw: namePrefix)MacroApplication(
            macroSystem: system,
            contextGenerator: contextGenerator,
            indentationWidth: indentationWidth
          )

          return \(raw: awaitOperator)applier.rewrite(self)
        }
        """
      )
    }
    ...
    DeclSyntax(
      ...
      private func expandFreestandingMemberDeclList(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) throws -> MemberBlockItemListSyntax? {
        guard
          let expanded = try \(raw: awaitOperator)expandFreestandingMacro(
    ...
    DeclSyntax(
      ...
      private func expandFreestandingCodeItemList(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) throws -> CodeBlockItemListSyntax? {
        guard
          let expanded = try \(raw: awaitOperator)expandFreestandingMacro(
            definition: definition,
            macroRole: inferFreestandingMacroRole(definition: definition),
    ...
    DeclSyntax(
      ...
      private func expandFreestandingExpr(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) throws -> ExprSyntax? {
        guard
          let expanded = \(raw: awaitOperator)expandFreestandingMacro(
            definition: definition,
...
private func makeMacroExpansionCodePart2(isAsync: Bool) -> CodeBlockItemListSyntax {
  let asyncSpecifier = if isAsync { " async" } else { String() }
  let awaitOperator = if isAsync { "await " } else { String() }
  ...
  return CodeBlockItemListSyntax() {
    DeclSyntax(
      ...
      private func expandMemberMacro(
        ...
        guard
          let expanded = \(raw: awaitOperator)expandAttachedMacro(
            definition: definition,
            macroRole: .member,
    ...
    DeclSyntax(
      ...
      private func expandMemberAttributeMacro(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) throws -> AttributeListSyntax? {
        guard
          let expanded = \(raw: awaitOperator)expandAttachedMacro(
            definition: definition,
    ...
    DeclSyntax(
      ...
      private func expandAccessorMacroWithoutExistingAccessors(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) throws -> AccessorBlockSyntax? {
        guard
          let expanded = \(raw: awaitOperator)expandAttachedMacro(
            definition: definition,
            macroRole: .accessor,
    ...
    DeclSyntax(
      ...
      private func expandAccessorMacroWithExistingAccessors(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) throws -> AccessorDeclListSyntax? {
        guard
          let expanded = \(raw: awaitOperator)expandAttachedMacro(
            definition: definition,
            macroRole: .accessor,
    ...
    DeclSyntax(
      ...
      private func expandPreambleMacro(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) -> CodeBlockItemListSyntax? {
        guard
          let expanded = \(raw: awaitOperator)expandAttachedMacro(
            definition: definition,
            macroRole: .preamble,
    ...
    DeclSyntax(
      ...
      private func expandBodyMacro(
        ...
        in context: some MacroExpansionContext,
        indentationWidth: Trivia
      )\(raw: asyncSpecifier) -> CodeBlockSyntax? {
        guard
          let expanded = \(raw: awaitOperator)expandAttachedMacro(
            definition: definition,
            macroRole: .body,
...
private func makeMacroApplicationCode(isAsync: Bool) -> CodeBlockItemListSyntax {
  let namePrefix = if isAsync { "Async" } else { String() }
  let asyncSpecifier = if isAsync { " async" } else { String() }
  let awaitOperator = if isAsync { "await " } else { String() }
  ...
  return CodeBlockItemListSyntax() {
    try! ClassDeclSyntax(
      """
      // MARK: - \(raw: namePrefix)MacroApplication

      /// Syntax rewriter that evaluates any macros encountered along the way.
      private class \(raw: namePrefix)MacroApplication<Context: MacroExpansionContext>: \(raw: namePrefix)SyntaxRewriter
      """
    ) {
      DeclSyntax("let macroSystem: MacroSystem")

      DeclSyntax("var contextGenerator: (Syntax) -> Context")

      DeclSyntax("var indentationWidth: Trivia")
      ...
      DeclSyntax(
        """
        override func visitAny(_ node: Syntax)\(raw: asyncSpecifier) -> Syntax? {
          ...
          switch \(raw: awaitOperator)expandExpr(node: node) {
          case .success(let expansion):
            return \(raw: awaitOperator)expansion.withExpandedNode { expandedNode in
              Syntax(\(raw: awaitOperator)visit(expandedNode))
            }
          case .failure:
            return Syntax(node)
          case .notAMacro:
            ...
            !attributedNode.attributes.isEmpty
          {
            ...
            {
              declSyntax = \(raw: awaitOperator)DeclSyntax(visitBodyAndPreambleMacros(declNodeWithBody))
            }

            // Visit the node, disabling the `visitAny` handling.
            skipVisitAnyHandling.insert(Syntax(declSyntax))
            let visitedNode = \(raw: awaitOperator)self.visit(declSyntax)
      ...
      DeclSyntax(
        ...
        func visitBodyAndPreambleMacros<Node: DeclSyntaxProtocol & WithOptionalCodeBlockSyntax>(
          ...
          // Expand body macro.
          let expandedBodies = \(raw: awaitOperator)expandMacros(
            attachedTo: DeclSyntax(node),
            ofType: BodyMacro.Type.self
          ) { attributeNode, definition, _ in
            \(raw: awaitOperator)expandBodyMacro(
              definition: definition,
              attributeNode: attributeNode,
              attachedTo: node,
              in: contextGenerator(Syntax(node)),
      ...
      DeclSyntax(
        """
        override func visit(_ node: CodeBlockItemListSyntax)\(raw: asyncSpecifier) -> CodeBlockItemListSyntax {
          var newItems: [CodeBlockItemSyntax] = []
          func addResult(_ node: CodeBlockItemSyntax)\(raw: asyncSpecifier) {
            // Expand freestanding macro.
            switch \(raw: awaitOperator)expandCodeBlockItem(node: node) {
            case .success(let expansion):
              \(raw: awaitOperator)expansion.withExpandedNode { expandedNode in
                for item in expandedNode {
                  \(raw: awaitOperator)addResult(item)
                }
              }
              return
            case .failure:
            ...
            case .notAMacro:
              ...
              newItems.append(\(raw: awaitOperator)visit(node))
            }

            // Expand any peer macro on this item.
            if case .decl(let decl) = node.item {
              for peer in \(raw: awaitOperator)expandCodeBlockPeers(of: decl) {
                \(raw: awaitOperator)addResult(peer)
              }
              extensions += \(raw: awaitOperator)expandExtensions(of: decl)
            }
          }
      ...
      DeclSyntax(
        """
        override func visit(_ node: MemberBlockSyntax)\(raw: asyncSpecifier) -> MemberBlockSyntax {
          let parentDeclGroup = node
            .parent?
            .as(DeclSyntax.self)
          var newItems: [MemberBlockItemSyntax] = []

          func addResult(_ node: MemberBlockItemSyntax)\(raw: asyncSpecifier) {
            // Expand freestanding macro.
            switch \(raw: awaitOperator)expandMemberDecl(node: node) {
            case .success(let expansion):
              \(raw: awaitOperator)expansion.withExpandedNode { expandedNode in
                for item in expandedNode {
                  \(raw: awaitOperator)addResult(item)
            ...
            for peer in \(raw: awaitOperator)expandMemberDeclPeers(of: node.decl) {
              \(raw: awaitOperator)addResult(peer)
            }
            extensions += \(raw: awaitOperator)expandExtensions(of: node.decl)
          }
          ...
          for var item in node.members {
            ...
            if let parentDeclGroup, let decl = item.decl.asProtocol(WithAttributesSyntax.self) {
              var attributes = \(raw: awaitOperator)expandAttributesFromMemberAttributeMacros(
                of: item.decl,
                parentDecl: parentDeclGroup
              )
              
              for i in 0..<attributes.count {
                attributes[i] = \(raw: awaitOperator)visit(attributes[i])
              }
              
              var newAttributes = AttributeListSyntax(attributes)
          ...
          if let parentDeclGroup {
            for member in \(raw: awaitOperator)expandMembers(of: parentDeclGroup) {
              \(raw: awaitOperator)addResult(member)
            }
          }
      ...
      DeclSyntax(
        """
        override func visit(_ node: VariableDeclSyntax)\(raw: asyncSpecifier) -> DeclSyntax {
          var node = \(raw: awaitOperator)super.visit(node).cast(VariableDeclSyntax.self)
          ...
          var expansion = \(raw: awaitOperator)expandAccessors(of: node, existingAccessors: binding.accessorBlock)
      ...
      DeclSyntax(
        """
        override func visit(_ node: SubscriptDeclSyntax)\(raw: asyncSpecifier) -> DeclSyntax {
          var node = \(raw: awaitOperator)super.visit(node).cast(SubscriptDeclSyntax.self)
          node.accessorBlock = \(raw: awaitOperator)expandAccessors(of: node, existingAccessors: node.accessorBlock).accessors
          return DeclSyntax(node)
        }
        """
      )
    ...
    try! ExtensionDeclSyntax(
      ...
      extension \(raw: namePrefix)MacroApplication
      """
    ) {
      DeclSyntax(
        """
      ...
      DeclSyntax(
        ...
        >(
          ...
          expandMacro:
            ...
            )\(raw: asyncSpecifier) throws -> ExpandedNodeCollection?
        )\(raw: asyncSpecifier) -> [ExpandedNode] {
          var result: [ExpandedNode] = []

          for macroAttribute in macroAttributes(attachedTo: decl, ofType: ofType) {
            do {
              if let expanded = try \(raw: awaitOperator)expandMacro(
      ...
      DeclSyntax(
        ...
        private func expandMemberDeclPeers(of decl: DeclSyntax)\(raw: asyncSpecifier) -> [MemberBlockItemSyntax] {
          return \(raw: awaitOperator)expandMacros(attachedTo: decl, ofType: PeerMacro.Type.self) { attributeNode, definition, conformanceList in
            return try \(raw: awaitOperator)expandPeerMacroMember(
              definition: definition,
              attributeNode: attributeNode,
              attachedTo: decl,
      ...
      DeclSyntax(
        ...
        ///
        /// - Returns: The macro-synthesized peers
        private func expandCodeBlockPeers(of decl: DeclSyntax)\(raw: asyncSpecifier) -> [CodeBlockItemSyntax] {
          return \(raw: awaitOperator)expandMacros(attachedTo: decl, ofType: PeerMacro.Type.self) { attributeNode, definition, conformanceList in
            return try \(raw: awaitOperator)expandPeerMacroCodeItem(
              definition: definition,
              attributeNode: attributeNode,
              attachedTo: decl,
              in: contextGenerator(Syntax(decl)),
      ...
      DeclSyntax(
        ...
        private func expandExtensions(of decl: DeclSyntax)\(raw: asyncSpecifier) -> [CodeBlockItemSyntax] {
          return \(raw: awaitOperator)expandMacros(
            attachedTo: decl,
            ofType: ExtensionMacro.Type.self
          ) { attributeNode, definition, conformanceList in
            return try \(raw: awaitOperator)expandExtensionMacro(
              definition: definition,
              attributeNode: attributeNode,
              attachedTo: decl,
              conformanceList: conformanceList,
      ...
      DeclSyntax(
        ...
        private func expandMembers(of decl: DeclSyntax)\(raw: asyncSpecifier) -> [MemberBlockItemSyntax] {
          return \(raw: awaitOperator)expandMacros(attachedTo: decl, ofType: MemberMacro.Type.self) { attributeNode, definition, conformanceList in
            return try \(raw: awaitOperator)expandMemberMacro(
              definition: definition,
              attributeNode: attributeNode,
              attachedTo: decl,
              conformanceList: conformanceList,
      ...
      DeclSyntax(
        ...
        )\(raw: asyncSpecifier) -> [AttributeListSyntax.Element] {
          return \(raw: awaitOperator)expandMacros(
            attachedTo: parentDecl,
            ofType: MemberAttributeMacro.Type.self
          ) { attributeNode, definition, conformanceList in
            return try \(raw: awaitOperator)expandMemberAttributeMacro(
              definition: definition,
              attributeNode: attributeNode,
              attachedTo: parentDecl,
      ...
      DeclSyntax(
        ...
        )\(raw: asyncSpecifier) -> (accessors: AccessorBlockSyntax?, expandsGetSet: Bool) {
          ...
          for macro in accessorMacros {
            do {
              ...
              if existingAccessors != nil {
                if let newAccessors = try \(raw: awaitOperator)expandAccessorMacroWithExistingAccessors(
              ...
              } else if let newAccessors = try \(raw: awaitOperator)expandAccessorMacroWithoutExistingAccessors(
                definition: macro.definition,
                attributeNode: macro.attributeNode,
                attachedTo: DeclSyntax(storage),
                in: contextGenerator(Syntax(storage)),
    ...
    try! ExtensionDeclSyntax(
      ...
      try! StructDeclSyntax(
        ...
        DeclSyntax("private let expandedNode: ResultType")

        DeclSyntax("private let macro: any Macro.Type")

        DeclSyntax("private unowned let macroApplication: \(raw: namePrefix)MacroApplication")

        DeclSyntax(
          """
          fileprivate init(expandedNode: ResultType, macro: any Macro.Type, macroApplication: \(raw: namePrefix)MacroApplication) {
        ...
        DeclSyntax(
          ...
          func withExpandedNode<T>(_ body: (_ expandedNode: ResultType)\(raw: asyncSpecifier) throws -> T)\(raw: asyncSpecifier) rethrows -> T {
            macroApplication.expandingFreestandingMacros.append(macro)
            defer {
              macroApplication.expandingFreestandingMacros.removeLast()
            }
            return try \(raw: awaitOperator)body(expandedNode)
          }
          """
        )
      }

      try! EnumDeclSyntax("enum MacroExpansionResult<ResultType>") {
        DeclSyntax(
          """
      ...
      DeclSyntax(
        ...
        private func expandFreestandingMacro<ExpandedMacroType: SyntaxProtocol>(
          ...
          expandMacro: (_ macro: any Macro.Type, _ node: any FreestandingMacroExpansionSyntax)\(raw: asyncSpecifier) throws -> ExpandedMacroType?
        )\(raw: asyncSpecifier) -> MacroExpansionResult<ExpandedMacroType> {
          guard let node,
          ...
          do {
            ...
            if let expanded = try \(raw: awaitOperator)expandMacro(macro, node) {
      ...
      DeclSyntax(
        ...
        /// }
        /// ```
        func expandCodeBlockItem(node: CodeBlockItemSyntax)\(raw: asyncSpecifier) -> MacroExpansionResult<CodeBlockItemListSyntax> {
          return \(raw: awaitOperator)expandFreestandingMacro(node.item.asProtocol(FreestandingMacroExpansionSyntax.self)) { macro, node in
            return try \(raw: awaitOperator)expandFreestandingCodeItemList(
              definition: macro,
              node: node,
              in: contextGenerator(Syntax(node)),
              indentationWidth: indentationWidth
      ...
      DeclSyntax(
        ...
        /// }
        /// ```
        func expandMemberDecl(node: MemberBlockItemSyntax)\(raw: asyncSpecifier) -> MacroExpansionResult<MemberBlockItemListSyntax> {
          return \(raw: awaitOperator)expandFreestandingMacro(node.decl.as(MacroExpansionDeclSyntax.self)) { macro, node in
            return try \(raw: awaitOperator)expandFreestandingMemberDeclList(
              definition: macro,
              node: node,
              in: contextGenerator(Syntax(node)),
              indentationWidth: indentationWidth
      ...
      DeclSyntax(
        ...
        /// let a = #foo
        /// ```
        func expandExpr(node: Syntax)\(raw: asyncSpecifier) -> MacroExpansionResult<ExprSyntax> {
          return \(raw: awaitOperator)expandFreestandingMacro(node.as(MacroExpansionExprSyntax.self)) { macro, node in
            return try \(raw: awaitOperator)expandFreestandingExpr(
              definition: macro,
              node: node,
              in: contextGenerator(Syntax(node)),
              indentationWidth: indentationWidth
